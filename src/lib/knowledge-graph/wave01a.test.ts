...
test("Integration QA is explicit and cannot authorize publication or cross-bind units", () => {
  ...
  assert.equal(lm013?.integrationQaState, "PASS");
  assert.equal(lm031?.integrationQaState, "PASS");
  assert.equal(lm013?.candidateRoute, "/proceso/leer-antes-de-aceptar");
  assert.equal(lm031?.candidateRoute, "/proceso/organizar-hechos-y-prueba");
  assert.notEqual(lm013?.candidateRoute, lm031?.candidateRoute);
  assert.equal(lm065?.candidateRoute, null);
  assert.equal(lm013?.publicationState, "NOT_PUBLIC");
  assert.equal(lm031?.publicationState, "NOT_PUBLIC");
  assert.equal(lm065?.publicationState, "NOT_PUBLIC");
});
...
test("live-state reconciliation invariants fail closed", () => {
  ...
  assert.equal(lm065?.integrationQaState, "NOT_RUN");
  assert.equal(lm065?.candidateRoute, null);
  assert.equal(lm065?.publicationState, "NOT_PUBLIC");
