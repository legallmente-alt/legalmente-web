...
export type LivingDictionaryEntry = {
  ...
  version: string;
  publicEligibilityState?: "SOURCE_BINDING_REQUIRED" | "LEGAL_REVIEW_REQUIRED" | "PUBLIC_DICTIONARY_ELIGIBLE" | "EXCLUDED" | "SUPERSEDED";
};
...
const ccfConsentBinding: SourceBinding = { id: "SB-MX-CONSENTIMIENTO-CCF-1803-2025", kind: "PRIMARY_LEGAL_SOURCE", authority: "Cámara de Diputados del H. Congreso de la Unión", title: "Código Civil Federal", territory: "México — contratos civiles sujetos al CCF", locator: "Artículos 1794, 1796, 1803 y 1812", url: "https://www.diputados.gob.mx/LeyesBiblio/pdf/CCF.pdf", articleRefs: ["1794", "1796", "1803", "1812"], reviewedAt: "2026-08-30", claimRefs: ["CCF-MX-CONSENTIMIENTO-1803-2025"], provenance: "VERIFIED" };

export const livingDictionary: readonly LivingDictionaryEntry[] = [
  { conceptId: "consentimiento", term: "Consentimiento", everydayNames: ["aceptar", "dar permiso", "estar de acuerdo", "qué estoy aceptando"], simpleDefinition: "En contratos civiles sujetos al Código Civil Federal mexicano, el consentimiento puede manifestarse de forma expresa o tácita.", technicalDefinition: "Para el ámbito contractual civil del CCF, el consentimiento expreso puede manifestarse verbalmente, por escrito, por medios electrónicos, ópticos u otra tecnología o por signos inequívocos; el tácito resulta de hechos o actos que lo presupongan, salvo que la ley o el convenio exijan manifestación expresa.", example: "Antes de aceptar una condición contractual, identifica quién la propone, qué autoriza y qué versión estás aceptando.", notToConfuseWith: ["firma", "silencio", "conocer un texto"], relatedConceptIds: ["obligacion", "prueba"], claimRefs: ["CCF-MX-CONSENTIMIENTO-1803-2025"], territory: "México — contratos civiles sujetos al CCF; no universal.", sources: [ccfConsentBinding, source("LM-CONCEPT-001", "Knowledge Graph LegalMente: consentimiento", "Panhispánico conceptual", "src/lib/knowledge-graph/content.ts#consentimiento")], limits: "No permite concluir por sí solo que una aceptación concreta sea válida, suficiente, exigible o aplicable fuera del ámbito contractual civil del CCF.", nextConceptId: "obligacion", relatedQuestions: ["¿Qué estoy aceptando?", "¿Qué versión cuenta?", "¿Puedo demostrar cómo acepté?"], version: "v1.1-mx", publicEligibilityState: "LEGAL_REVIEW_REQUIRED" },
...
export function getPublicDictionaryEligibility(entry: LivingDictionaryEntry) {
  ...
  return entry.publicEligibilityState === "PUBLIC_DICTIONARY_ELIGIBLE" && Boolean(getConcept(entry.conceptId) && entry.simpleDefinition && entry.technicalDefinition && entry.claimRefs?.length && hasLegalBinding && entry.territory && entry.limits && entry.version);
}
