...
for cid, u in units.items():
    lines += [f'### FOUNDER-{cid}', '', f'**FOUNDER_POST_ID:** `FOUNDER-{cid}-01`', f'**RELATED_CONTENT_ID:** `{cid}`', f'**ANGLE:** Separar descripción, clasificación y decisión antes de operar.', f'**CORPORATE_PROBLEM:** La empresa puede confundir una categoría general o un documento con una conclusión sobre una relación, entidad o contrato concreto.', f'**DECISION_CONTEXT:** Preparar una revisión ordenada con hechos, fuente, territorio, documento y evidencia pertinente.', f'**LEGAL_CONCEPT:** {u["concept_label"]}; fuente: {u["source"]}.', f'**BUSINESS_RISK:** Decidir con una etiqueta incompleta puede ocultar diferencias entre documento, operación, relación, entidad y hechos.', f'**WHAT_TO_REVIEW:** {u["previous"]} {u["next"]}', f'**LIMIT:** {u["qualifier"]} Información educativa, no asesoría individual.', f'**SOURCE:** {u["source"]} — {u["source_url"]}', f'**CTA / QUESTION:** ¿Qué capa conviene separar primero en esta revisión: hechos, documento, clasificación o relación?', f'**VISUAL_4X5:** `assets/{cid}_feed_4x5.png`', '']
...
(ROOT / 'current-state.json').write_text(json.dumps({cid: {
    ...
    'next_gate': 'Human product decision per unit; integration QA; publication decision' if cid == 'LM-PC-013' else 'Founder/Editor row-level mapping to a semantically correct existing parent or explicitly related-only record; then product decision; integration QA; publication decision',
